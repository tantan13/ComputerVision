...
class HistogramLayer(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        ** You will receive extra-credit if you successfully implement this
        ...
        mg = torch.unsqueeze(torch.norm(im_grads, dim=1), 0)
        mg = mg.repeat(1,8,1,1)
        N, c, w, h = x.size()
        dim2 = x.shape[2]
        dim3 = x.shape[3]
        bot = torch.zeros(8,dim2,dim3)
        for i in range(dim2):
            for j in range(dim3):
                index = torch.argmax(cosines[:,:,i,j],1)
                bot[index,i,j] = 1
        bot = torch.unsqueeze(bot,0)
        per_px_histogram = torch.mul(bot,mg)
...
class SubGridAccumulationLayer(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.layer = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=4,
            bias=False, padding=2, padding_mode='zeros', groups=8)
...
class SIFTOrientationLayer(nn.Module):
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter():
        ...
        #######################################################################
                ...
                #midpoints of bins
        omfg = [np.pi/8, (3*np.pi)/8, (5*np.pi)/8, (7*np.pi)/8, (9*np.pi)/8, (11*np.pi)/8, (13*np.pi)/8, (15*np.pi)/8, 0, np.pi/2]
        
        omfg = torch.tensor(omfg).unsqueeze(-1).unsqueeze(-1)
        weight_param = nn.Parameter(angles_to_vectors_2d_pytorch(omfg))
#         self.layer = nn.Conv2d(in_channels=2, out_channels=10, kernel_size=1,
#             bias=False)
#         self.layer.weight = weight_param
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
        return weight_param
...
class SIFTNet(nn.Module):
    ...
    def __init__(self):
        ...
        subgrid_acc_layer = SubGridAccumulationLayer()
        sift_layer = SIFTOrientationLayer()
        
        self.net = nn.Sequential(image_gradients_layer, sift_layer, histogram_layer, subgrid_acc_layer)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def get_sift_subgrid_coords(x_center: int, y_center: int):
    ...
    ###########################################################################
    # TODO: YOUR CODE HERE            
    #get_sift_subgrid_coords: Get x and y coordinates for the centers of each 4x4 subgrid. 
    ###########################################################################
    
#   torch.argmax(cosines[:,:,i,j],1)
    #print(x_center)
    #print(y_center)
    
#     Use arange() for this. I used bounds x_center - 7 and x_center + 6 to get my accuracy to work in the end. You can again use meshgrid() and flatten() to get the final tensors. 
    x_centers = []
    y_centers = []
    for i in range(16):
        if i <= 3:
            y_centers.append(y_center - 6)
        elif i <= 7:
            y_centers.append(y_center - 2)
        elif i <= 11:
            y_centers.append(y_center + 2)
        else: 
            y_centers.append(y_center + 6)
    j = 0
    while j <= len(y_centers) - 4:
        x_centers.append(x_center - 6)
        x_centers.append(x_center - 2)
        x_centers.append(x_center + 2)
        x_centers.append(x_center + 6)
        j+=4
   

    x_grid = np.array(x_centers)
    y_grid = np.array(y_centers)
#     print(x_centers)
#     print(y_centers)
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    img = net.forward(img_bw) 
    vectors = []
    for i in range(len(x)):
         outputs = []
         x_centers, y_centers = get_sift_subgrid_coords(x[i],y[i])
         for j in range(len(x_centers)):
            outputs.append(img[:,:,y_centers[j],x_centers[j]].detach().numpy())
         outputs = np.array(outputs)
         outputs = outputs.reshape((1,-1))
         outputs = outputs/outputs.sum()
         outputs = np.power(outputs, 0.9)
         vectors.append(outputs)
    
    fvs = np.array(vectors)
    fvs = fvs.reshape((len(x),128))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################

...
class HybridImageModel(nn.Module):
  ...
  def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
    ...
    - You can use np.reshape(1, k ,k) to change the dimensions of a numpy array.
    ...
    k = len(kernel)
    kernel = torch.tensor(np.tile(kernel.reshape((1, 1, k, k)), (self.n_channels, 1, 1, 1)), dtype=torch.float)
    ### END OF STUDENT CODE ####
    ############################
  ...
  def low_pass(self, x, kernel):
    ...
    filtered_image = F.conv2d(x, kernel, padding=(kernel.size()[2]//2, kernel.size()[3]//2), groups=self.n_channels)
  ...
  def forward(self, image1, image2, cutoff_frequency):
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    k1 = self.get_kernel(cutoff_frequency)

    low_frequencies = self.low_pass(image1, k1)
    low_f2 = self.low_pass(image2, k1)
    high_frequencies = image2 - low_f2

...
def make_dataset(path: str) -> Tuple[List[str],List[str]]:
  ...
  images_a = [path + '/' + i for i in image_files if i[i.index('_') - 1] == 'a']
  images_b = [path + '/' + i for i in image_files if i[i.index('_') - 1] == 'b']
...
def get_cutoff_frequencies(path: str) -> List[int]:
  ...
  ### TODO: YOUR CODE HERE ###
  cutoff_frequencies = []
  text_files = open(path, 'r')

  for file in text_files.readlines():
      cutoff_frequencies.append(int(file))
  text_files.close()
  ### END OF STUDENT CODE ####
  ############################
...
class HybridImageDataset(data.Dataset):
  ...
  def __getitem__(self, idx: int) -> Tuple[torch.Tensor,torch.Tensor,int]:
    ...
    should read the images here and convert them to
    torch Tensors.
